...
public class ProjectService {
	 ...
	 */
	@Transactional
	public void addProjectAndSkills(int resumeSeq, List<ProjectEntity> projects, List<List<ProjectTechStackEntity>> projectSkills) {
	...
	/**
	 * 해당 프로젝트의 기술 스택들 가져오기
	 * @param resumeSeq
	 * @return
	 */
	public List<List<ProjectTechStackDTO>> searchProjectStackList(int resumeSeq){
		
		List<List<ProjectTechStackDTO>> pstDTOList = new ArrayList<>();
		
		List<ProjectDTO> pDTOList = searchProjectByResume(resumeSeq);
		for(ProjectDTO pDTO : pDTOList) {
			int projectSeq = pDTO.getProjectSeq();
			List<ProjectTechStackDTO> pstDTO = pMapper.selectProjectStackByresume(projectSeq);
			pstDTOList.add(pstDTO);
		}
		
		return pstDTOList;
	}//searchProjectStackList
}
